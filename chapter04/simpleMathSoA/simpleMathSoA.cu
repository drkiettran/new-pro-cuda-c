...
{
    std::cout << "simpleMathSoA program starts ..." << std::endl;
    ...
    std::cout << "warmup2: " << GetDurationInMicroSeconds(begin, StopTimer()) << " microsecs" << std::endl;
    ...
    CHECK(cudaDeviceSynchronize());
    std::cout << "testInnerArray: " << GetDurationInMicroSeconds(begin, StopTimer()) << " microsecs" << std::endl;
    CHECK(cudaMemcpy(gpuRef, d_C, nBytes, cudaMemcpyDeviceToHost));
    checkInnerArray(hostRef, gpuRef, nElem);
    CHECK(cudaGetLastError());
