 ...
 *           uses a basic binary tree reduction.  It also uses shared
 *           memory to store intermediate results.  Assumes both 
 *           threads_per_block and blocks_per_grid are powers of 2.
 *
 * Compile:  nvcc  -arch=sm_21 -o dot2 dot2.cu 
 * Run:      ./dot2 <n> <blocks> <threads_per_block>
